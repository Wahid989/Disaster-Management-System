/*
Copyright Ann Barcomb and Khawla Shnaikat, 2024
Licensed under GPL v3
See LICENSE.txt for more information.
*/
package edu.ucalgary.oop;

import org.junit.Test;
import static org.junit.Assert.*;

import java.time.LocalDate;

public class SupplyTest {
    String expectedType = "Blanket";
	int expectedQuantity = 5;
LocalDate expectedDateAdded = LocalDate.now();
    LocalDate expectedExpiryDate = LocalDate.now().plusDays(30);
    private Supply supply = new Supply(expectedType, expectedQuantity, expectedDateAdded, expectedExpiryDate);

/*
testObjectCreation -> means testing Supply constructor: 
   - What we need: To verify that an "Supply" object is successfully created.
   - Actual result: Supply type is "Blanket", The quantity is 5.
   - Expected Result: The test checks that the Supply object is not null, confirming successful object creation.
*/
    @Test
    public void testObjectCreation() {
        assertNotNull(supply);
    }
	
/*testGetFirstName: 
   - What we need: To ensure the "getType()" method correctly returns the actual Supply type.
   - Actual result: "Blanket".
   - Expected result: "supply.getType()" should return "Blanket".
*/
    @Test
    public void testGetType() {
        assertEquals("getType should return the correct type", expectedType, supply.getType());
    }
/*testSetType(): 
   - What we need: To ensure the "setType()" method correctly updates the Supply type.
   - Actual/current result: "Blanket".
   - Expected result: "Food".
*/
    @Test
    public void testSetType() {
        supply.setType("Food");
        assertEquals("setType should update the type", "Food", supply.getType());
    }

/*testGetQuantity(): 
   - What we need: To ensure the "getQuantity()" method correctly returns the actual Supply quantity.
   - Actual result: 5.
   - Expected result: "supply.getType()" should return "Blanket".
*/
    @Test
    public void testGetQuantity() {
        assertEquals("getQuantity should return the correct quantity", expectedQuantity, supply.getQuantity());
    }

/*testSetQuantity(): 
   - What we need: To ensure the "setQuantity" method correctly updates the Supply quantity.
   - Actual/current result: 5.
   - Expected result: 20.
*/
    @Test
    public void testSetQuantity() {
        supply.setQuantity(20);
        assertEquals("setQuantity should update the quantity", 20, supply.getQuantity());
    }

    @Test
    public void testConstructor() {
        String expectedType = "Blanket";
        int expectedQuantity = 5;
        LocalDate expectedDateAdded = LocalDate.now();
        LocalDate expectedExpiryDate = LocalDate.now().plusDays(30);
        
        Supply supply = new Supply(expectedType, expectedQuantity, expectedDateAdded, expectedExpiryDate);

        assertNotNull("Supply object should not be null", supply);
        assertEquals("Type should match the expected value", expectedType, supply.getType());
        assertEquals("Quantity should match the expected value", expectedQuantity, supply.getQuantity());
        assertEquals("Date added should match the expected value", expectedDateAdded, supply.getDateAdded());
        assertEquals("Expiry date should match the expected value", expectedExpiryDate, supply.getExpiryDate());
    }

    // Testing setDateAdded and getDateAdded methods
    @Test
    public void testSetAndGetDateAdded() {
        LocalDate currentDate = LocalDate.now();
        LocalDate newDate = LocalDate.now().plusDays(15);

        Supply supply = new Supply("Food", 10, currentDate, LocalDate.now().plusDays(30));
        supply.setDateAdded(newDate);

        assertEquals("Date added should match the new value", newDate, supply.getDateAdded());
    }

    // Testing setExpiryDate and getExpiryDate methods
    @Test
    public void testSetAndGetExpiryDate() {
        LocalDate currentDate = LocalDate.now();
        LocalDate newExpiryDate = LocalDate.now().plusDays(45);

        Supply supply = new Supply("Water", 20, LocalDate.now(), currentDate);
        supply.setExpiryDate(newExpiryDate);

        assertEquals("Expiry date should match the new value", newExpiryDate, supply.getExpiryDate());
    }
}
